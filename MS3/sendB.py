...
def splitRange(addr, iface, lower, upper, size = 10):
    ...
    while i < upper - size:
        ...
        pkt = pkt / IP(dst=addr, proto=KVSQUERY_PROTOCOL) / KVSQuery(protocol=TCP_PROTOCOL, queryType=2, key=i, key2=i+size, clientID = 1)
        # for j in range(4):
    ...
    pkt = Ether(src=get_if_hwaddr(iface), dst='ff:ff:ff:ff:ff:ff', type=RESPONSE_PROTOCOL)
    pkt = pkt / Response(nextType = 1)
    pkt = pkt / IP(dst=addr, proto=KVSQUERY_PROTOCOL) / KVSQuery(protocol=TCP_PROTOCOL, key=i, key2=upper, queryType=2, clientID = 1)
